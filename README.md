...
Furthermore, we want those classes to have (read-only!) properties that can be used to track the coroutine progress. We suggest allowing visibility modifiers `public` and `internal` for top-level `var`s and `val`s as well as the ones in labeled blocks and labeled loops. Those would generate read-only properties in the respective Job subclasses. For computed properties, `public/internal get val`s should be introduced.
